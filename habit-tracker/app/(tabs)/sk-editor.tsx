...
const BUNDLE_KEY = "editor_bundle_extracted_v1";
const URL_KEY = "editor_custom_url";
const BASE_DIR = () => (FileSystem.documentDirectory as string) + "webapps/editor/";
...
async function extractBundle(): Promise<string> {
  ...
  const bundle: Record<string, string> = require("../../assets/editor-bundle.json");
...
export default function SkEditorScreen() {
  ...
  useEffect(() => {
    AsyncStorage.multiGet([URL_KEY, "editor_use_custom"]).then(pairs => {
      const url = pairs[0][1] ?? "";
  ...
  function saveUrl() {
    ...
    setCustomUrl(u);
    setUseCustom(!!u);
    AsyncStorage.setItem("editor_use_custom", u ? "true" : "false");
    setEditUrl(false);
  }
  ...
  function toggleSource() {
    ...
    setUseCustom(next);
    AsyncStorage.setItem("editor_use_custom", next ? "true" : "false");
  }
    ...
    <View style={[s.root, { backgroundColor: BG }]}>
      <View style={[s.header, { paddingTop: insets.top + 8 }]}>
        <View style={s.left}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.title}>SK Code Editor</Text>
          ...
          <View style={{ flex: 1, gap: 6 }}>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TextInput
                ...
                placeholder="https://seu-app.replit.app/sk-code-editor/"
      ...
      {extracting ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyTitle}>Preparando SK Code Editor...</Text>
